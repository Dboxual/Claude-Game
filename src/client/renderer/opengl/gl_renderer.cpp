...
bool GLRenderer::init(IWindow& window) {
    ...
    // UI rect pipeline.
    rectProg_ = compileProgram(kRectVS, kRectFS);
    if (!rectProg_) return false;
    locRectScreen_ = glGetUniformLocation(rectProg_, "uScreen");

    glGenVertexArrays(1, &rectVao_);
    glGenBuffers(1, &rectVbo_);
    glBindVertexArray(rectVao_);
    glBindBuffer(GL_ARRAY_BUFFER, rectVbo_);
    const GLsizei rectStride = 6 * sizeof(float);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, rectStride, (const void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, rectStride, (const void*)(2 * sizeof(float)));
    glEnableVertexAttribArray(1);
    glBindVertexArray(0);
    return true;
}
...
void GLRenderer::shutdown() {
    ...
    if (rectProg_) glDeleteProgram(rectProg_);
    rectVbo_ = rectVao_ = rectProg_ = 0;
...
void GLRenderer::drawRects(const RenderFrame& frame) {
    if (frame.rects.empty()) return;

    rectVerts_.clear();
    for (const RectDraw& r : frame.rects) {
        const float x0 = r.x, y0 = r.y, x1 = r.x + r.w, y1 = r.y + r.h;
        const float quad[6][6] = {
            {x0, y0, r.color.r, r.color.g, r.color.b, r.color.a},
            {x1, y0, r.color.r, r.color.g, r.color.b, r.color.a},
            {x1, y1, r.color.r, r.color.g, r.color.b, r.color.a},
            {x0, y0, r.color.r, r.color.g, r.color.b, r.color.a},
            {x1, y1, r.color.r, r.color.g, r.color.b, r.color.a},
            {x0, y1, r.color.r, r.color.g, r.color.b, r.color.a},
        };
        rectVerts_.insert(rectVerts_.end(), &quad[0][0], &quad[0][0] + 36);
    }
    ...
    glUniform2f(locRectScreen_, float(frame.viewportW), float(frame.viewportH));
    glBindVertexArray(rectVao_);
    glBindBuffer(GL_ARRAY_BUFFER, rectVbo_);
    glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(rectVerts_.size() * sizeof(float)),
                 rectVerts_.data(), GL_DYNAMIC_DRAW);
    glDrawArrays(GL_TRIANGLES, 0, GLsizei(rectVerts_.size() / 6));
    glBindVertexArray(0);
